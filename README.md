...
Each gate activates exactly its own branch: with the default config the executed plan (135 rules of 363 total) is identical to the pre-branch port, and toggling one key adds only that branch's rules (verified by `dry-run` per key).
