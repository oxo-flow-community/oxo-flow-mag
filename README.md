...
- **Reference data you must provide:**
  - Your reads, declared either in `config.input_dir` as paired-end `{sample}_R1.fastq.gz` / `{sample}_R2.fastq.gz` files (default `test/fixtures/raw`, which ships tiny test fixtures) or, for mixed layouts, via `config.reads_sheet` — a samplesheet TSV with columns `sample, reads_1, reads_2, reads_layout` (`pe` | `single` | `interleaved`), mirroring the upstream `--input` samplesheet (an example ships as `test/fixtures/reads_sheet.tsv`). Uniform single-end libraries can also be run by overriding `config.sample_pattern`. Multi-lane samples (a sample sequenced over several lanes) are declared as one sheet row per lane with lane-suffixed ids (`S1_lane1`, `S1_lane2`, ...) — after preprocessing the lanes are concatenated into the canonical merged paths and assembly runs on the merged base id (see [Multi-lane merging](#multi-lane-merging)).
...
Upstream MERGE_LANES (`subworkflows/local/preprocessing_shortread`) merges a sample's lanes AFTER all preprocessing (clip → host removal → phiX removal): lanes are distinct sheet rows (`S1_lane1`, `S1_lane2`), each goes through the QC chain separately (FASTQC_TRIMMED stays per-lane, pre-merge — exactly like upstream), and then `CAT_FASTQ` (`groupTuple` + branch `reads.size() >= 2`) concatenates the phiX-removed mates into the canonical merged paths. The port mirrors this with two discovery-driven rules, `concat_lane_mate1` / `concat_lane_mate2`: their `input_groups` pattern matches only producer outputs of lane-named sheet ids (`{base}_lane*_run1_phix_removed_{1,2}.unmapped.fastq.gz`, grouped by `{base}`), so they instantiate exactly for samples with ≥2 lanes and emit the canonical `{base}_run1_phix_removed_{1,2}` files; single-lane samples never match and skip the concat entirely (like upstream).

Contract:

- One sheet row per lane, id `{base}_lane{n}`, any layout per row; the merged base id (`S1`) has NO sheet row and NO metadata row.
- Add the lanes to the sample set; add the merged base too — it schedules assembly/binning via the `|| {meta.reads_1} == ''` disjunct on the concatenated canonical paths (the engine treats `metadata_file` as a lookup only, and `{meta.*}` renders '' for rowless ids, which is exactly what the disjunct keys on).
- Do NOT add the base id to `cohort_samples`: upstream merges lanes before binning prep, so the align loop reads only merged paths.
...
| Eukaryotic gene prediction | `metaeuk_mmseqs_db = "path/to/db"` and/or `metaeuk_db = "path/to/db"` | 3 | `METAEUK_EASYPREDICT` + `MMSEQS_DATABASES` (metaeuk 6.a5d39d9, mmseqs2 18.8cc5c; the MMseqs db archive/directory is unpacked by `metaeuk_mmseqs_db_preparation`, the path is the gate) |
| Long-read assembly and binning | `long_reads = "reads.fastq.gz"` (+ `long_reads_platform`) | 10 | `FLYE` + `MetaMDBG` + minimap2 binning prep + MetaBAT2/MaxBin2/CONCOCT (see `modules/12_longreads.oxoflow`) |
| Long-read QC chain + host removal | `long_reads_qc = true` (+ `longread_adaptertrimming_tool` / `longread_filtering_tool`) | 10 | `NANOPLOT_RAW` → `PORECHOP_ABI`/`PORECHOP` (ONT) → `NANOLYSE` (unless chopper) → `CHOPPER`/`NANOQ`/`FILTLONG` → `MINIMAP2_HOST_INDEX`/`MINIMAP2_HOST_ALIGN`+`SAMTOOLS_HOSTREMOVED_UNMAPPED` (host_fasta) → `NANOPLOT_FILTERED` (see `modules/13_longread_qc.oxoflow`; upstream defaults mirrored: `porechop_abi`, `chopper`) |
...
- **Long-read preprocessing QC** (chopper / nanoq / nanolyse / porechop(+abi) / filtlong / nanoplot; upstream `preprocessing_longread`, QC on by default — `skip_longread_qc=false`): ported as `modules/13_longread_qc.oxoflow`, gated by `long_reads_qc = true` — the port keeps the pre-existing default (QC-passed FASTQ) and opts into the chain; the upstream defaults for the trimming/filtering tools are mirrored (`porechop_abi`, `chopper`) and the lambda reference is bundled (`assets/data/GCA_000840245.1_...fna.gz`, upstream projectDir asset).
- **Long-read host removal** (upstream `hostremoval_longread`, minimap2 + samtools chain): ported in the same module — activates with `long_reads_qc = true` when `host_fasta` is supplied (`MINIMAP2_HOST_INDEX` → `MINIMAP2_HOST_ALIGN` → `SAMTOOLS_HOSTREMOVED_UNMAPPED`; minimap2 presets from the platform like upstream MINIMAP2_HOST_ALIGN).
- **Filtlong filtered-by-shortreads mode** (upstream `filtlong_filtering_by_shortreads`): the port's long-read chain filters by long-read Phred scores only (upstream default); the short-reads-aided mode requires joining long/short streams per sample, which the port's single-FASTQ long-read shape does not model.
- **SPAdesHybrid hybrid assembly** (upstream `assembly_hybrid` / METASPADESHYBRID, `skip_spadeshybrid=false` default): ported as `spades_hybrid` — gated on `config.long_reads` + `config.skip_spadeshybrid = false` (the port ships the branch off like the other gated branches); `--meta --nanopore` (ONT) / `--meta --pacbio` (PacBio) by platform, exactly the upstream branch on `lr_platform`.
- **pypolca polish** (upstream `PYPOLCA_RUN` of long-read assemblies, `--run_pypolca`): ported as `pypolca` — gated on `config.long_reads` + `config.run_pypolca`; polishes the Flye and MetaMDBG assemblies with the sample's phiX-removed short reads (`--careful`, upstream default); outputs land in `intermediates/assembly/pypolca/` (upstream publishes under `Assembly/<assembler>-pypolca/`).
- **MetaEuk eukaryotic gene prediction** (upstream `metaeuk/easypredict` + `mmseqs/databases`): ported as `modules/14_metaeuk.oxoflow` — gated on a non-empty `config.metaeuk_mmseqs_db` (MMseqs database archive or unpacked directory, unpacked/copied by `metaeuk_mmseqs_db_preparation`) or `config.metaeuk_db` (FASTA/HH-suite database path); the path is the gate (cat_db precedent). `metaeuk easy-predict` runs per bin + unbinned chunk on all binners (SPAdes + MEGAHIT variants), proteins to `Annotation/MetaEuk/<assembler>/` like the upstream publishDir. Deviation: upstream filters bins to Tiara `eukarya`/`unclassified` domains when `--bin_domain_classification` is on — the port runs MetaEuk on ALL bins like the shipped PROKKA rules (documented above); the zero-bin guard touches the declared outputs.
...
- **Multi-lane merging** (upstream MERGE_LANES / CAT_FASTQ, `subworkflows/local/preprocessing_shortread`): now ported — sheet rows per lane + the `concat_lane_mate{1,2}` rules concatenate phiX-removed lanes into the canonical merged paths (see [Multi-lane merging](#multi-lane-merging)). Note: upstream `SEQTK_MERGEPE` is a different, `--mergemergedreads`-pair merge; the lane merge itself is CAT_FASTQ concat, which is what the port implements.
...
**Live queue** — the branches ported in the catalog truthification wave (2026-08-27) needed real databases for live verification; results after the box-queue live-testing campaign (2026-09, bioinfo-wsx):
